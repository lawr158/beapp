...
for curr_file=1:length(grp_proc_info_in.beapp_fname_all)
    ...
    if exist(grp_proc_info_in.beapp_fname_all{curr_file},'file')
        load(grp_proc_info_in.beapp_fname_all{curr_file}, 'eeg_wfp', 'file_proc_info')
        tic;
        if exist('eeg_wfp','var')
            ...
            disp(['no eeg_w variable found in ' file_proc_info.beapp_fname ', fooof not calculated']) %disp(['no usable segments were found in ' file_proc_info.beapp_fname ', fooof not calculated']);
        end
    end
end
