...
for curr_file=1:length(grp_proc_info_in.beapp_fname_all)
        ...
        if exist('eeg_w','var')
            ...
            for curr_condition = 1:size(curr_eeg,1)
                ...
                if ~isempty(curr_eeg{curr_condition,1})
                    ...
                    if size(curr_cond_eeg,3) > (grp_proc_info_in.bycyc_num_segs-1)
                        ...
                        if ~isempty(grp_proc_info_in.win_select_n_trials)
                            ...
                            if grp_proc_info_in.bycyc_set_num_segs==1
                                segments_torun = randsample(size(curr_cond_eeg,3),grp_proc_info_in.bycyc_num_segs);
                            else
                                segments_torun = [1:size(curr_cond_eeg,3)];
                            end
                        end
                            ...
                            if ismember(chan,file_proc_info.beapp_indx{1,1})
                                ...
                                for freq_idx = 1:size(frequency_bands,1)
                                    ...
                                    for seg = 1:size(segments_torun,2) 
                                        ...
                                        signal = curr_cond_eeg(chan,:,curr_seg); 
    ...
    %                             %results_table_nburst(:,chan) = squeeze(nanmean(nanmean(curr_chan_results_nburst,1),3));
            ...
            disp(['no usable segments were found in ' file_proc_info.beapp_fname ', bycycle not calculated']);
        end
    end
